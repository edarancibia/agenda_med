...
public interface EventJpaRepository extends JpaRepository<Events2, Long> {
	...
	@Query(value="select e.id as id,p.dni,e.rut_num, e.start as start,e.end as end,e.description,CONCAT(p.nombre,' ',p.a_pat,' ',p.a_mat) as title,estado,e.rut_num as rut from events e,paciente p where e.rut_num=p.dni and e.rut_med = :rut_med and e.estado in(1,2)",nativeQuery=true)
